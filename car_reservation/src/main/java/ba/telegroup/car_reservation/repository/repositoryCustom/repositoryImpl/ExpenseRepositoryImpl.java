...
public class ExpenseRepositoryImpl extends CustomRepositoryImpl implements ExpenseRepositoryCustom {
    private String SQL_GET_ALL_EXTENDED_BY_RESERVATION_ID="select e.id,e.cost_id,e.company_id,c2.cost,e.user_id,u.username,concat(u.first_name,' ',u.last_name) as full_name,e.reservation_id,e.price,e.date,e.description,e.car_id,concat(c.plate_number,' - ',m2.name,' ',m.model) as car_model,e.deleted from expense e inner join reservation r on e.reservation_id = r.id inner join user u on r.user_id = u.id inner join car c on r.car_id = c.id inner join model m on c.model_id = m.id inner join manufacturer m2 on m.manufacturer_id = m2.id inner join cost c2 on e.cost_id = c2.id where r.id=? and e.deleted=0 order by date desc,price desc";

    private String SQL_GET_ALL_EXTENDED="select e.id,e.cost_id,e.company_id,c2.cost,e.user_id,u.username,concat(u.first_name,' ',u.last_name) as full_name,e.reservation_id,e.price,e.date,e.description,e.car_id,concat(c.plate_number,' - ',m2.name,' ',m.model) as car_model,e.deleted from expense e inner join reservation r on e.reservation_id = r.id inner join user u on r.user_id = u.id inner join car c on r.car_id = c.id inner join model m on c.model_id = m.id inner join manufacturer m2 on m.manufacturer_id = m2.id inner join cost c2 on e.cost_id = c2.id where e.deleted=0 order by date desc,price desc";

    private String SQL_GET_EXTENDED_BY_ID="select e.id,e.cost_id,e.company_id,c2.cost,e.user_id,u.username,concat(u.first_name,' ',u.last_name) as full_name,e.reservation_id,e.price,e.date,e.description,e.car_id,concat(c.plate_number,' - ',m2.name,' ',m.model) as car_model,e.deleted from expense e inner join reservation r on e.reservation_id = r.id inner join user u on r.user_id = u.id inner join car c on r.car_id = c.id inner join model m on c.model_id = m.id inner join manufacturer m2 on m.manufacturer_id = m2.id inner join cost c2 on e.cost_id = c2.id where e.deleted=0 and e.id=? order by date desc,price desc";

    private String SQL_GET_EXTENDED_BY_CAR_ID="select e.id,e.company_id,e.cost_id,c2.cost,e.user_id,u.username,concat(u.first_name,' ',u.last_name) as full_name,e.reservation_id,e.price,e.date,e.description,e.car_id,concat(c.plate_number,' - ',m2.name,' ',m.model) as car_model,e.deleted from expense e inner join reservation r on e.reservation_id = r.id inner join user u on r.user_id = u.id inner join car c on r.car_id = c.id inner join model m on c.model_id = m.id inner join manufacturer m2 on m.manufacturer_id = m2.id inner join cost c2 on e.cost_id = c2.id where e.deleted=0 and e.car_id=? order by date desc,price desc";
    ...
    private final String SQL_DELETE_BY_CAR="update expense u set u.deleted=1 where car_id=?";

    private final String SQL_DELETE_BY_RESERVATION="update expense u set u.deleted=1 where reservation_id=?";
    ...
    @Override
    public Long deleteByReservationId(Integer reservationId) {
        return Long.valueOf(entityManager.createNativeQuery(SQL_DELETE_BY_RESERVATION).setParameter(1,reservationId).executeUpdate());
    }
}
